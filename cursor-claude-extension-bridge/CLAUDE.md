...
The **MCP tool bridge** has two halves because the CLI can't call VS Code APIs: `mcp/toolBridgeServer.ts` runs a token-protected loopback HTTP endpoint inside the extension host exposing `vscode.lm` tools (feature-detected; absent in some Cursor builds), and `mcp/mcpBridge.ts` is a standalone stdio MCP server the CLI launches via a generated `--mcp-config`, proxying `tools/list`/`tools/call` to that endpoint. This is why `esbuild.js` produces **two separate bundles** — `dist/mcp-bridge.js` must remain independently runnable (it's spawned as its own process).
